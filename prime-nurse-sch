...
> 2. **Full Mastery:** 1동 팀도 특수파트(G1, G7)를 모두 포함하여 1동 전체 완전 정복
> 3. **Skill Matrix:** 데이터 시각화를 통한 조직 역량 관리 (엑셀 자동화)
> 4. **Smart Dispatch:** 긴급 상황 시 최적의 인력 즉시 추천
""")

# --- 병동 그룹 데이터 (최종 수정: 1동에도 G7 추가) ---

# 1동 Team (Generalist + G1, G7 특수파트 모두 포함 -> 7개 그룹)
structure_general = {
    ...
    "G5(1동_9층)": ["91W", "92W"],
    "G6(호흡기/종양)": ["122W", "131W"],
    "✨G1(소아/산과)": ["41W", "51W"], # 1동 특수 1
    "✨G7(격리/특수)": ["82W"]          # 1동 특수 2 (추가됨!)
}

# 2동 Team (Specialist + G1, G7 지원 -> 6개 그룹)
structure_special = {
